...
const DisspaceLogo = () => {
  return (
    <Link href="/">
      <a>
        <Image src={logoImg} alt="" height={63} width={153} />
      </a>
    </Link>
  );
};
...
const SearchBar = () => {
  ...
  return (
    <form className="flex" onSubmit={handleSearch}>
      <button
        className="border-y-2 border-l-2 border-orange px-2 pt-1 rounded-l h-8"
        type="submit"
      >
      ...
      try {
        getDownloadURL(
          ref(
            storage,
            returnImgString(
              "gs://disspace-76973.appspot.com/user_profile_img/profile_default.jpg"
            )
          )
        ).then((url) => {
          setImg(url);
  ...
  return (
    <div className="flex flex-row gap-1">
      <div className="mt-2 flex flex-row gap-3">
        <Link href="/create/">
          <a>
            <div className="h-8 w-8 mt-[2px] border-orange rounded-md">
      ...
      {/* <Link href={`/userProfile/${user.username}`}>
                <a> */}
      <Menu>
        <Menu.Button className="flex flex-row h-12 w-auto max-w-[170px] border-2 border-orange rounded-md text-center font-normal p-1">
          ...
          <div className="ml-3 mt-1">{user.username}</div>
        </Menu.Button>
        <Transition
          as={Fragment}
        ...
        >
          <Menu.Items className="absolute w-32 mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
